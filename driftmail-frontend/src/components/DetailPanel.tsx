...
export default function DetailPanel() {
  ...
  if (!selectedEmailId || !email) {
    return (
      <div className="w-[420px] flex-shrink-0 bg-[#F5F4F0] flex flex-col items-center justify-center text-slate-400 gap-4 border-l border-slate-100">
        <div className="w-16 h-16 rounded-3xl bg-white shadow-card flex items-center justify-center text-3xl">
          ✉️
        </div>
        <div className="text-center">
          <p className="text-sm font-medium text-slate-600">Select an email</p>
          <p className="text-xs mt-1">Click any row to see details and ask questions</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="w-[420px] flex-shrink-0 flex flex-col bg-white border-l border-slate-100 overflow-hidden"
      >
        {/* Header */}
        <div className="px-5 py-4 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <div className="flex gap-1.5 flex-shrink-0">
              ...
              <button
                onClick={() => selectEmail(null)}
                className="haptic text-slate-400 hover:text-slate-600 transition-colors ml-1"
              >
                ✕
              </button>
            </div>
          ...
          {email.sender && (
            <p className="text-xs text-slate-500 mb-2">From: {email.sender}</p>
          )}
        ...
        <div className="flex-shrink-0 border-t border-slate-100">
          <div className="px-5 pt-3 pb-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-600">Ask about this email</span>
            </div>
          </div>
          <AskInput emailId={selectedEmailId} onAsked={scrollToBottom} />
        </div>
